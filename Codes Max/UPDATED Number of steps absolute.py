...
for u in range(version):
    ...
    for i in range(training_times):
            ...
            # Load loss values
            loss_values_train = torch.load("fake_loss_curve.pt")
            fig = plt.figure()
            ...
            fig, ax = plt.subplots()
            plt.plot(number_of_steps, loss_values_train)
            ax.set_ylabel("Loss value (-)")
            ax.set_xlabel("Time steps (-)")
            fig.suptitle(f"Fake filtered loss curve with cliff at 700 time steps")
            fig.savefig("fakefilteredlosscurve.jpg")
            ...
            torch.save(number_of_steps_in_plateau, f"ABS_Steps_per_plateau_2nd_bs{batch_size}_w{width}_hl{hidlay}_v{u+1}_ds{dataset_size}_e{epochs}_tt{training_times}.pt")
