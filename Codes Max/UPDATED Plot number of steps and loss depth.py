...
for u in range(version):
    for i in range(training_times):
        steps = torch.load(f"ABS_Steps_per_plateau_2nd_bs{batch_size}_w{width}_hl{hidlay}_v{u+1}_ds{dataset_size}_e{epochs}_tt{training_times}.pt")
    ...
    fig.savefig(f"Escape steps 2nd w{width}_hl{hidlay}_v{u+1}_ds{dataset_size}_e{epochs}_tt{training_times}.pdf")
