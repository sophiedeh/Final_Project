...
width = 10 #amount of nodes per layer
hidlay = 6 #amount of layers
different_depth = 1
different_width = 1
...
for i in range(training_times): 
    for u in range(version):
        ...
        plt.show()
        fig.savefig(f"Plot_2nd_bs{batch_size}_w{width}_hl{hidlay}_v{u+1}_ds{dataset_size}_e{epochs}_tt{training_times}.pdf")
        ...
        torch.save(final_losses_train,f"Final_losses_train_2nd_bs{batch_size}_w{width}_hl{hidlay}_v{u+1}_ds{dataset_size}_e{epochs}_tt{training_times}.pt")
        torch.save(minimum_losses_test,f"Minimum_losses_test_2nd__bs{batch_size}_w{width}_hl{hidlay}_v{u+1}_ds{dataset_size}_e{epochs}_tt{training_times}.pt")
        torch.save(loss_values_train, f"Loss_values_train_2nd_bs{batch_size}_w{width}_hl{hidlay}_v{u+1}_ds{dataset_size}_e{epochs}_tt{training_times}.pt")
    batch_size += 25
    #width += 10